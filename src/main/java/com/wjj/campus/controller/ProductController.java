...
public class ProductController {
    ...
    @ResponseBody
    public JsonResponse modifyProduct(HttpServletRequest request,
                                      @RequestParam("product") String product,
                                      String verifyCode,
                                      @RequestParam(value = "thumbnail", required = false) MultipartFile thumbnail,
                                      @RequestParam(value = "productDetailsPicture", required = false) MultipartFile[] productDetailsPicture) throws UnsupportedEncodingException {
        ...
        logger.debug("开始修改产品信息...");
        return productService.modifyProduct(p, productThumbnail, fileContainerList);
    }
